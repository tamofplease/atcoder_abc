...
int main() {
    ...
    rep(i, m) {
        ...
        cin >> l >> r >> d, l--, r--;
        ds.merge(l, r);
        right[l].push_back({r, d});
        right[r].push_back({l, -d});
    }
    bool ans = true;
    vector<ll> position(n, INT64_MIN);
    auto dfs = [&](auto f, int cur) -> void {
        for (const auto [nxt, dist] : right[cur]) {
            if (position[nxt] == INT64_MIN) {
                position[nxt] = position[cur] + dist;
                f(f, nxt);
                continue;
            }
            if (position[nxt] != position[cur] + dist) {
                ans = false;
                break;
            }
        }
    };
    for (vector<int>& d : ds.groups()) {
        position[d[0]] = 0;
        dfs(dfs, d[0]);
    }
    if (ans)
