...
int main() {
    ...
    rep(i, m) {
        ...
        cin >> u >> v, u--, v--;
        if (nodes.same(u, v)) {
            edges[nodes.leader(u)]++;
            continue;
        }
        int edge = edges[nodes.leader(u)] + edges[nodes.leader(v)] + 1;
        nodes.merge(u, v);
        edges[nodes.leader(u)] = edge;
    }
    for (vector<int>& v : nodes.groups()) {
        int node_size = v.size();
        int edge_size = edges[nodes.leader(v[0])];
